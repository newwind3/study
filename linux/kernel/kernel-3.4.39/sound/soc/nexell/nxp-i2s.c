...
///////////////// pop noise 2nd
struct timer_list g_hpmute_timer;
#define HPMUTE_TIMER_INTERVAL msecs_to_jiffies(100)
...
#ifndef CFG_AUDIO_I2S0_EXTERNAL_MASTER
...
static int i2s_start(struct nxp_i2s_snd_param *par, int stream)
{
	...
	printk("%s: CON %08x CSR %08x FIC %08x PSR %08x\n", __func__, i2s->CON, i2s->CSR, FIC, i2s->PSR);
...
extern void akaudio_set_mute_dac(int mute);
extern void akaudio_set_mute_amp(int mute);
extern void akaudio_set_power_dac(int on);
extern void akaudio_set_power_opamp(int on);
static int first_start_i2s = 0;

static void g_hpmute_timer_work(unsigned long data)    
{
	printk("g_hpmute_timer_work \n");
	akaudio_set_mute_amp(0);
}

static char* nxp_i2s_cmdtostr(int cmd)
{
	switch (cmd) {
				...
				int cmd, struct snd_soc_dai *dai)
{
	...
	pr_debug("%s: %s cmd= %s\n", __func__, STREAM_STR(stream), nxp_i2s_cmdtostr(cmd));
...
//////////////////////////////////// pop noise 2nd 
	akaudio_set_mute_dac(0);	

	if (first_start_i2s==0)   
	{
		init_timer(&g_hpmute_timer);
		g_hpmute_timer.function = g_hpmute_timer_work;
		g_hpmute_timer.expires = jiffies + HPMUTE_TIMER_INTERVAL;	
		add_timer(&g_hpmute_timer);
		first_start_i2s = 1;
	}
	else
    		mod_timer(&g_hpmute_timer, jiffies + HPMUTE_TIMER_INTERVAL);
///////////////////////////////////////////
...
/////////////////////////////////////////// power off noise 1st
	akaudio_set_mute_amp(1);
	mdelay(100);
	akaudio_set_mute_dac(1);
///////////////////////////////////////////
...
static int nxp_i2s_set_dfs_sysclk(struct nxp_i2s_snd_param *par)
{
	...
	int OEN = (i2s->CSR >> CSR_CDCLKCON_POS) & 0x1;
#else
	int OEN = 0;
#endif
	...
	//i2s_reset(par); //yys-DDR533으로 낮추면서 특정단말 노이즈 발생 
	...
	dmap_play->real_clock = rate_hz/(RATIO_256==RFS?256:384);
	dmap_capt->real_clock = rate_hz/(RATIO_256==RFS?256:384);
	pr_debug("%s snd i2s: ch %d, %s, %s mode, %d(%ld)hz, %d FBITs, MCLK=%ldhz, RFS=%d\n", __func__, 
		par->channel, par->master_mode?"master":"slave",
		par->trans_mode==0?"iis":(par->trans_mode==1?"left justified":"right justified"),
		par->sample_rate, rate_hz/(RATIO_256==RFS?256:384),
		par->frame_bit, rate_hz, (RATIO_256==RFS?256:384));
	pr_debug("%s snd i2s: BLC=%d, IMS=%d, LRP=%d, SDF=%d, RFS=%d, BFS=%d\n", __func__, BLC, IMS, LRP, SDF, RFS, BFS);
...
static int nxp_i2s_dai_resume(struct snd_soc_dai *dai)
{
	...
	if (par->master_mode && par->in_clkgen && par->clk_rate) {
