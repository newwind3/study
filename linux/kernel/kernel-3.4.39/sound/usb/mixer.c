				  ...
				  struct snd_kcontrol *kctl)
{
	switch (cval->mixer->chip->usb_id) {
		//J17
		case USB_ID(0x21b4, 0x0083):/*Dragonfly*/
			printk("--DRAGONFLY ATTACHED--\n");
			printk("--%s: dbMin[%d], dbMax[%d], min[%d],max[%d],res[%d]--\n", kctl->id.name, cval->dBmin, cval->dBmax, cval->min, cval->max, cval->res);
			cval->max = 100;
			cval->min = 0;
			cval->res = 1;
			break;
		case USB_ID(0x245f, 0x1213): /*Hugo*/
		case USB_ID(0x245f, 0x0815): /*Mojo*/
