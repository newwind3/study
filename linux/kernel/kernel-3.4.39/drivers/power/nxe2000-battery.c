...
extern struct  switch_dev ak_usb_mode_dev;
#endif
////////////////////////////////////////
//  Debug
////////////////////////////////////////
...
#endif
#define FULL_LEVEL_ADJUSTMENT 98
#define LOW_LEVEL_ADJUSTMENT 0
extern char *saved_command_line; //yys- 
...
int g_old_capacity=-1;
int g_low_bat_state = 0;
int g_old_time=0, g_gap_time=0, g_gap_sum_time=0, g_gap_count=0, g_gap_flag=0;
int g_old_voltage=0;

static int g_charger_boot=0;
static int fake_bat_enable=0; 
struct nxe2000_battery_info *g_info;
...
#endif
extern int ak_get_boot_complete(void);
#endif
...
static int calib_ocvTable(struct nxe2000_battery_info *info, int vbat_ocv)
{
	...
	printk("\033[01;33m[%s]vbat_ocv:%d\033[00m\n", __FUNCTION__ , vbat_ocv);
...
static void nxe2000_sw_ubc_work(struct work_struct *work)
{
	...
	if (pmic_vbus && !info->flag_set_ilimit) {
		printk("is_sdp_type:%d curr_reg:0x%x\n",info->is_sdp_type, temp);		
...
//	info->first_pwon = (val == 0) ? 1 : 0;
	...
	info->first_pwon = (val == 0) ? 1 : 0;
 	...
 	printk("%s: info->first_pwon:%d (%d)\n",__func__, info->first_pwon, val);
...
struct wake_lock usb_host_mode_lock;
int ct10_set_otg_power_en(int enable)
{
	if(enable) {
	//	if(AK_USB_MODE_HOST != switch_get_state(&ak_usb_mode_dev))
	//		return -1;

		gpio_set_value(CFG_GPIO_OTG_5V_EN, 1);
		gpio_set_value(CFG_GPIO_OTG_VBUS_EN, 1);
		wake_lock(&usb_host_mode_lock);
	} else {
		gpio_set_value(CFG_GPIO_OTG_5V_EN, 0);
		gpio_set_value(CFG_GPIO_OTG_VBUS_EN, 0);
		wake_unlock(&usb_host_mode_lock);
	}

	return 0;

}
EXPORT_SYMBOL(ct10_set_otg_power_en);

int otgid_power_control_by_dwc(int enable)
{
#if 1 //J17
	if(!ak_get_boot_complete())
		return 0;
	/*return ct10_set_otg_power_en(enable);*/
#endif
	if(info_by_dwc == NULL)
		return -ENODEV;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		soc_org = info->capacity;
		//yys-
		//info->capacity = (((soc_org-LOW_LEVEL_ADJUSTMENT)*100)*100)/((FULL_LEVEL_ADJUSTMENT-LOW_LEVEL_ADJUSTMENT)*100);

		//only test      
	    if(fake_bat_enable){
		    info->capacity  = fake_bat_level;
		}
		
		//printk("\033[01;33myys[%s(%d,old:%d)] \033[00m\n", __FUNCTION__,info->capacity, g_old_capacity);
		
		soc_gap = info->capacity - g_old_capacity;	
		vol_gap = info->cur_voltage - g_old_voltage;

		ch_state = gpio_get_value(CFG_GPIO_PMIC_VUSB_DET);

		if(info->cur_voltage != g_old_voltage){
			if(!ch_state && vol_gap < 0){
				printk("char:%d vol:%dmv, old:%dmv, vol_gap:%d\n",ch_state ,info->cur_voltage ,g_old_voltage ,vol_gap);	
			}
		}

		printk("soc_gap:%d\n", soc_gap);
		if(info->capacity != g_old_capacity){
			g_gap_time = elapsed_jiffies_msecs(g_old_time)/1000; //sec
			g_old_time = jiffies;	
				
			printk("\033[01;33myys-Battery [soc:%d%, old:%d%, %dmv, soc_gap:%d time:%02d:%02d sum:%d]\033[00m\n",
					info->capacity, g_old_capacity, info->cur_voltage, soc_gap, g_gap_time/60, g_gap_time%60, g_gap_sum_time);

			if(soc_gap < 0){//soc_gap이 -이면 방전중...
				if(g_gap_time < 60 && g_gap_sum_time <60){
				//	info->capacity = g_old_capacity;
					g_gap_count++;
					g_gap_sum_time += g_gap_time;
					g_gap_flag = 1;
					printk("do compensation battery soc:%d%, sum:%d\n", info->capacity, g_gap_sum_time);
				}else{
					g_gap_count = 0; 
					g_gap_sum_time = 0;
					g_gap_flag = 0;
		...
		/*
		if(info->capacity != g_old_capacity){
			printk("\033[01;33mBattery [%d% , %d mV, %d minutes] \033[00m\n",info->capacity, info->cur_voltage);
			//g_old_capacity = info->capacity;
		}
		*/
		
		g_old_capacity = info->capacity;
		g_old_voltage= info->cur_voltage;
...
#endif
		if(info->capacity == 0 && !get_lcd_state() && g_low_bat_state == 0){
			printk("\033[01;33mBattery 0%->Power off\033[00m\n");
			g_low_bat_state = 1;
			nxp_key_power_event();
		}
		DBGOUT(info->dev, "battery capacity is %d(%d)%\n",
							info->capacity, soc_org);
...
static ssize_t force_pswr_init_show(struct device *dev, struct device_attribute *attr, char *buf)
{               
	printk("%s\n",__FUNCTION__);
	return 0;
}   
static ssize_t force_pswr_init_sotre(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{              
	uint8_t val;
	if(*buf == '1'){
		printk("\033[01;33myys[%s(%d)] \033[00m\n", __FUNCTION__, __LINE__);
		nxe2000_read(g_info->dev->parent, PSWR_REG, &val);
		printk("%s +PSWR_REG: %d\n",__FUNCTION__,val);
		nxe2000_write(g_info->dev->parent, PSWR_REG, 0);
		nxe2000_read(g_info->dev->parent, PSWR_REG, &val);
		printk("%s -PSWR_REG: %d\n",__FUNCTION__,val);
		//nxp_cpu_reset('h',NULL);
	}         
	return count;
}    
static DEVICE_ATTR(battery_pswr_init, 0777, force_pswr_init_show, force_pswr_init_sotre ); 
...
{ 
    ...
    //create "sys/class/iriver/battery/battery_pswr_init"
    //ex)  cd sys/class/iriver/battery
    //     cat battery_pswr_init  ==> battery_show                                                                                                                                                        
    //       echo "1" > battery_pswr_init ==> battery_store
    ret = device_create_file(dev, &dev_attr_battery_pswr_init);   
    if (unlikely(ret < 0))      
        pr_err("battery_create_file: failed to create device file, %s\n", dev_attr_battery_pswr_init.attr.name);
    
    //create "sys/class/iriver/battery/fake_battery_level"
    //ex)  cd sys/class/iriver/battery
    //     cat fake_battery_level   ==> fake_battery_level_show
    //      echo 10 > /sys/class/iriver/battery/fake_battery_level 
...
static __devinit int nxe2000_battery_probe(struct platform_device *pdev)
{
	...
	if (strstr(saved_command_line, "androidboot.mode=charger")) { //yys-
		printk("androidboot.mode=chargeri\n");
		g_charger_boot=1;
	}else{
		printk("androidboot.mode=normal\n");
		g_charger_boot=0;
	}
	...
	if(g_charger_boot == 1)
		info->ch_ilim_dcp	= 0xE0 | 0x0F; //yys-charg mode current
	else
	info->ch_ilim_dcp	= 0xE0 | (info->ch_ilim_adp ); //yys-
...
//	nxe2000_power_suspend_status	= 0;
	...
	wake_lock_init(&usb_host_mode_lock, WAKE_LOCK_SUSPEND, "usb_host_mode_lock");
...
static void nxe2000_battery_shutdown(struct platform_device *pdev)
{
	struct nxe2000_battery_info *info = platform_get_drvdata(pdev);
	uint8_t val = 0;
	int temp, ret;
	...
	if (info->is_sdp_type)
		...
		temp = (val & 0xE0) | 0x0F;//0x0F:1600mA  ILIM_ADP

	printk("%s: is_sdp_type:%d curr_reg:0x%x\n",__func__, info->is_sdp_type, temp);		
	ret = nxe2000_write(info->dev->parent, NXE2000_REG_REGISET2, temp);
	if (ret < 0)
		ret = nxe2000_write(info->dev->parent, NXE2000_REG_REGISET2, temp);
...
static struct platform_driver nxe2000_battery_driver = {
	...
	.shutdown = nxe2000_battery_shutdown,
};
