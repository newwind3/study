...
extern struct  switch_dev ak_usb_func_dev;
static int     g_cur_usb_function = AK_USB_DEVICE_FUNC_NONE;
//J17-
...
static void switch_usb_monitor(struct work_struct *data)
{
	struct android_dev *dev = _android_dev;//container_of(data, struct android_dev, work);

	//J17
	if(dev->connected) {
		switch_set_state(&ak_usb_mode_dev, AK_USB_MODE_DEVICE);
		if(g_cur_usb_function == AK_USB_DEVICE_FUNC_MTP) 
			switch_set_state(&ak_usb_func_dev, AK_USB_DEVICE_FUNC_MTP);
	}
	else {
		switch_set_state(&ak_usb_func_dev, AK_USB_DEVICE_FUNC_NONE);
		switch_set_state(&ak_usb_mode_dev, AK_USB_MODE_NONE);
	}
			       ...
			       struct usb_configuration *c)
{
	...
	list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
		...
		//J17
		if(f == &mtp_function) {
			g_cur_usb_function = AK_USB_DEVICE_FUNC_MTP;
			       ...
			       struct usb_configuration *c)
{
	...
	list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
		...
		//J17
		if(f == &mtp_function) {
			g_cur_usb_function = AK_USB_DEVICE_FUNC_NONE;
			//switch_set_state(&ak_usb_mode_dev, AK_USB_MODE_NONE);
		}
	}
}
...
static int __init init(void)
{
	...
	INIT_WORK(&dev->mon, switch_usb_monitor);
