...
static unsigned int g_pwr_io = 0;

void ak_power_key_control(int mode)
{
	if (mode)
		enable_irq(gpio_to_irq(g_pwr_io));		
	else
		disable_irq(gpio_to_irq(g_pwr_io));
}
EXPORT_SYMBOL(ak_power_key_control);
...
void ak_key_playpause_event(void)
{
	if (key_input) {
		input_report_key(key_input, KEY_PLAYPAUSE, 1);
    	input_sync(key_input);
		input_report_key(key_input, KEY_PLAYPAUSE, 0);
    	input_sync(key_input);
    }
}
...
extern int nxp_check_pm_wakeup_alive(int num);
...
static int nxp_key_resume(struct platform_device *pdev)
{
	...
	/* [downer/A1710106] for SD wakeup */
	if (nxp_check_pm_wakeup_alive(PAD_GPIO_ALV + 1)) {
		printk("SD wakeup\n");
		nxp_key_power_event();
	}
	else if (nxp_check_pm_wakeup_alive(PAD_GPIO_ALV + 3)) { /* [downer/A171115] for Play/Pause */
		printk("Play/Pause wakeup\n");
		ak_key_playpause_event();
	}
...
static int nxp_key_probe(struct platform_device *pdev)
{
	...
	for (i=0; keys > i; i++, code++) {
		...
		if (code->keycode == KEY_POWER) /* [downer/A171106] */
			g_pwr_io = code->io;
