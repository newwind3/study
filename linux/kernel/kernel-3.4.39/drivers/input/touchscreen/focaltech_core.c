...
/* [downer] A171120 for TSP point up */
#define TOUCH_RELEASE_TIMEOUT         (jiffies + 50)
...
static int fts_power_source_init(struct fts_ts_data *data)
{
    ...
    data->vdd = regulator_get(NULL, "vtouch_3.3V");
	if (IS_ERR(data->vdd))
	{
		rc = PTR_ERR(data->vdd);
	...
	{
		FTS_INFO("TSP set voltage\n");
		rc = regulator_set_voltage(data->vdd, FTS_VTG_MAX_UV, FTS_VTG_MAX_UV);
		if (rc)
		{
...
static void ft_touch_release_event(unsigned long data)
{
...
static void fts_report_value(struct fts_ts_data *data)
{
                ...
                for (i = 0; i < data->pdata->key_number; i++)
                {
                        ...
                        event->au16_x[0] < (data->pdata->key_x_coords[i] + FTS_KEY_WIDTH))
                    {
						...
						if (event->au8_touch_event[i]== 0 ||
							...
							if (event->au16_x[0] == 520 && event->au16_y[0] == 900) {
								input_report_key(data->input_dev, KEY_HOMEPAGE, 1);
								FTS_DEBUG("HOME Key%d(%d, %d) DOWN!", i, event->au16_x[0], event->au16_y[0]);
								tsp_button = 1;
							}
							else {
    ...
    for (i = 0; i < event->touch_point; i++) {
		if (event->au8_touch_event[i] == FTS_TOUCH_DOWN || event->au8_touch_event[i] == FTS_TOUCH_CONTACT) {
			if (event->au16_x[i] < SCREEN_MAX_X && event->au16_y[i] < SCREEN_MAX_Y) {
				...
				if (FTS_REPORT_PRESSURE_EN)	{
					FTS_DEBUG("[A]P%d(%d, %d)[p:%d,tm:%d] DOWN!", event->au8_finger_id[i], event->au16_x[i], event->au16_y[i], pressure, event->area[i]);
					mod_timer(&data->release_timer, TOUCH_RELEASE_TIMEOUT); /* [downer/A171120] */
				}
				else {
				...
				if (FTS_REPORT_PRESSURE_EN)	{
					FTS_DEBUG("[H]P%d(%d, %d)[p:%d,tm:%d] DOWN!", event->au8_finger_id[i], event->au16_x[i], event->au16_y[i], pressure, event->area[i]);
				
					if (event->au16_x[i] == 520 && event->au16_y[i] == 900) {
						FTS_DEBUG("REPORT HOME!\n");
						//						uppoint++;
						tsp_button = 1;
						input_report_key(data->input_dev, KEY_HOMEPAGE, 1);
						input_mt_sync(data->input_dev);										
    ...
    if (event->touch_point == uppoint) {
		if (tsp_button) {
			FTS_DEBUG("HOME Up!(%d)", tsp_button);			
			tsp_button = 0;
			...
			//			FTS_DEBUG("Points All Up!(%d)", tsp_button);
			mod_timer(&data->release_timer, TOUCH_RELEASE_TIMEOUT); /* [downer/A171120] */
			//			input_report_key(data->input_dev, BTN_TOUCH, 0);
		}
	}
    else {
		FTS_DEBUG("##### (%d)", event->touch_point);
		if (tsp_button) 
			input_report_key(data->input_dev, KEY_HOMEPAGE, event->touch_point > 0);
		else
			mod_timer(&data->release_timer, TOUCH_RELEASE_TIMEOUT);	/* [downer/A171120] */
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
     ...
     * touch release timer
     */
    setup_timer(&data->release_timer, ft_touch_release_event, (unsigned long)data);
