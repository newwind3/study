...
void fts_update_fw_vendor_id(struct fts_ts_data *data)
{
	...
	printk("\x1b[1;33m\n[FT5X46] FW vendor ID is 0x%x\x1b[0m\n", data->fw_vendor_id);	
    if (err < 0)
...
bool fts_ctpm_check_need_upgrade(struct i2c_client *client)
{
    ...
    if (fw_status == FTS_RUN_IN_APP) //call fts_flash_get_upgrade_info in probe function firstly.
    {
		...
		printk("\x1b[1;33m[FT5X46] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\x1b[0m\n", uc_tp_fm_ver, uc_host_fm_ver);
#if 1
		if (uc_tp_fm_ver < uc_host_fm_ver )
#else
		if (1)					/* Firmware force update */
#endif
        {
            fts_updatefun_curr.need_upgrade = true;
            bUpgradeFlag = true;
			printk("\x1b[1;33m[FT5X46] Need TSP FW upgrade.\x1b[0m\n");
        }
        else
        {
            fts_updatefun_curr.need_upgrade = false;
			printk("\x1b[1;38m[FT5X46] No need TSP FW upgrade\x1b[0m\n");			
        }
