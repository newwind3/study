...
#ifdef CUT_OFF_3000V	
#define BAT_IMP			2488	//bat_impe=0.2488
#define CUT_OFF_V 	3000	//cut-off_v=3300
#endif
#ifdef CUT_OFF_3100V	
#define BAT_IMP			3354	//bat_impe=0.3354
#define CUT_OFF_V 	3350	//cut-off_v=3350
#endif
#ifdef CUT_OFF_3200V	
#define BAT_IMP			3146	// bat_impe=0.3146
#define CUT_OFF_V 	3450	//cut-off_v=3300
#endif
#ifdef CUT_OFF_3300V
#define BAT_IMP			2700	//bat_impe=0.2700
#define CUT_OFF_V 	3300	//cut-off_v=3300
#endif
#ifdef CUT_OFF_3350V
#define BAT_IMP			3314	//bat_impe=0.3314
#define CUT_OFF_V 	3350	//cut-off_v=3300
#endif
#ifdef CUT_OFF_3450V	
#define BAT_IMP			2626	//bat_impe=0.2626
#define CUT_OFF_V 	3450	//cut-off_v=3300
#endif
#ifdef CUT_OFF_LEPUS
#define BAT_IMP		1891	
#define CUT_OFF_V 	3450	
#endif
/*------------------------------------------------------------------------------
 * BUS Configure
 */
...
//extern int touch_power_enable(int on);
static void lcd_bl_notify_after(struct device *dev, int brightness)
{
///	printk("====== backlight changed: %d\n", brightness);
//	touch_power_enable(brightness);
}
...
static struct platform_pwm_backlight_data bl_plat_data = {
	...
	.notify_after	= lcd_bl_notify_after,
};
...
struct nxp_ts_cali_plat_data ts_plat_data = {
	.touch_points	= 10,
	.x_resol	   	= 480,  ///CFG_DISP_PRI_RESOL_WIDTH,		// FIXME
	.y_resol	   	= 800, ///CFG_DISP_PRI_RESOL_HEIGHT,		// FIXME
	.rotate			= 0,
};
...
#elif defined(CONFIG_TOUCHSCREEN_FTS)
#include <linux/i2c.h>
#define	FT5446_I2C_BUS		(1)
struct nxp_ts_cali_plat_data ts_plat_data = {
	.touch_points	= 10,
	.x_resol	   	= 480,  ///CFG_DISP_PRI_RESOL_WIDTH,		// FIXME
	.y_resol	   	= 800, ///CFG_DISP_PRI_RESOL_HEIGHT,		// FIXME
...
static struct i2c_board_info __initdata ft5446_i2c_bdi[] = {
	{
		.type	= "fts_ts",
		.addr	= (0x70>>1),
		.irq    = PB_PIO_IRQ(CFG_IO_TOUCH_PENDOWN_DETECT),
...
static struct rotary_encoder_platform_data ak_gpio_encoder_info = {
    .steps  = 200,
    .axis   = ABS_VOLUME,
    .half_period = 1,
/* this was moved to probe function in rotary_encoder.c
...
static struct platform_device ak_gpio_encoder_device = {
    .name   = "gpio-encoder",
    .id     = 0,
    .dev    = {
        .platform_data = &ak_gpio_encoder_info,
    },
};
#endif

#if defined(CONFIG_SWITCH_GPIO_AK)
...
/* gpio i2c 3 */
#define I2C3_SCL    ((PAD_GPIO_E + 21) | PAD_FUNC_ALT0)
#define I2C3_SDA    ((PAD_GPIO_E + 22) | PAD_FUNC_ALT0)
...
#define NXE2000_CHARGING_PATH	INPUT_POWER_TYPE_UBC
//#define NXE2000_CHARGING_PATH	INPUT_POWER_TYPE_ADP_NOBATTERY
...
static struct regulator_consumer_supply nxe2000_ldo1_supply_0[] = {
	REGULATOR_SUPPLY("lcd_1.8V", NULL), //yys-
};
static struct regulator_consumer_supply nxe2000_ldo2_supply_0[] = {
	REGULATOR_SUPPLY("lcd_2.8V", NULL),	//yys-
};
...
#else
NXE2000_PDATA_INIT(ldo1,     0,	1000000, 3500000, 0, 0, 1800000, 1,  2);	/* 1.8V LCD_1V8 */
#endif
NXE2000_PDATA_INIT(ldo2,     0,	1000000, 3500000, 0, 0, 2800000, 1,  2);	/* 2.8V LCD_2V8*/
NXE2000_PDATA_INIT(ldo3,     0,	1000000, 3500000, 1, 1, 1800000, 1,  2);	/* 1.8V SYS1 */
NXE2000_PDATA_INIT(ldo4,     0,	1000000, 3500000, 1, 1, 1900000, 1,  2);	/* 1.9V SYS */
...
NXE2000_PDATA_INIT(ldo8,     0,	1000000, 2800000, 0, 1, 2800000, 1,  0);	/* 2.8V TOUCH */
NXE2000_PDATA_INIT(ldo9,     0,	1000000, 3500000, 0, 0,      -1, 0,  0);	/* Not Use */
NXE2000_PDATA_INIT(ldo10,    0,	1000000, 3500000, 1, 1, 1000000, 1,  0);	/* yys-DVDD_1V0 */
...
static struct nxe2000_battery_platform_data nxe2000_battery_data = {
	...
	.input_power_type	= NXE2000_CHARGING_PATH,
	...
	.type[0] = {
		...
		.ch_ichg 		= 0x0E,	/* ICHG		= 0 - 0x1D (100mA - 3000mA) */			// CHGISET_REG (LOW)		
		.ch_ichg_slp	= 0x0E,	/* SLEEP  ICHG	= 0 - 0x1D (100mA - 3000mA) */		// CHGISET_REG (LOW)
		.ch_ilim_adp 	= 0x0C,	/* ILIM_ADP	= 0 - 0x1D (100mA - 3000mA) 0x0D:1400mA*/			// REGISET1
		.ch_ilim_usb 	= 0x06,	/* ILIM_USB	= 0 - 0x1D (100mA - 3000mA) 0x04:500mA*/			// REGISET2
		.ch_icchg		= 0x00,	/* ICCHG	= 0 - 3 (50mA 100mA 150mA 200mA) */		// CHGISET_REG (HIGH) yys-충전 전류가 50mA 이하일때 완충   

		.fg_target_vsys	= 3250,	/* This value is the target one to DSOC=0% */
		.fg_target_ibat	= 1000,	/* This value is the target one to DSOC=0% */
		.fg_poff_vbat	= 3500,	/* setting value of 0 per Vbat */
...
static int _dwmci2_init(u32 slot_id, irq_handler_t handler, void *data)
{
	...
	int id  = slot_id, ret = 0;

	printk("dw_mmc dw_mmc.%d: Using external card detect irq %3d (io %2d)\n", id, irq, io);

	ret = request_irq(irq, handler, IRQ_TYPE_EDGE_BOTH | IRQF_SHARED,
...
{
	return nxp_soc_gpio_get_in_value(CFG_SDMMC2_DETECT_IO);
}

static struct dw_mci_board _dwmci2_data = {
    .quirks         = DW_MCI_QUIRK_HIGHSPEED,
    .bus_hz         = 100 * 1000 * 1000,
//  .hs_over_clk    = xx * 1000 * 1000,
//	.caps			= MMC_CAP_UHS_DDR50 | MMC_CAP_CMD23,
	.caps			= MMC_CAP_CMD23,
    .detect_delay_ms= 200,
...
#define FACTORY_BDADDR_STORAGE_SIZE		64

char bluetooth_mac_addr[FACTORY_BDADDR_STORAGE_SIZE];
EXPORT_SYMBOL(bluetooth_mac_addr);

int nxp_plat_get_bluetooth_addr(void *context, int max_size)
{
	int nsize = min(FACTORY_BDADDR_STORAGE_SIZE, max_size);
	memcpy(context, bluetooth_mac_addr, nsize);
	return nsize;
}
EXPORT_SYMBOL(nxp_plat_get_bluetooth_addr);

static void eeprom_setup_get_mac_addr(struct memory_accessor *mem_acc, void *context)
{
	char addr[256];
	off_t offset = (off_t)context;

	// Read MAC addr from EEPROM
	// setprop ro.bt.bdaddr_path "/system/etc/bluetooth/bd_addr.txt"
	memset(bluetooth_mac_addr, 0x00, FACTORY_BDADDR_STORAGE_SIZE);

	if (mem_acc->read(mem_acc, addr, offset, FACTORY_BT_BDADDR_STORAGE_LEN) == FACTORY_BT_BDADDR_STORAGE_LEN) {
		memcpy(bluetooth_mac_addr, addr, FACTORY_BT_BDADDR_STORAGE_LEN);
		printk("Read MAC from EEPROM: %s\n", bluetooth_mac_addr);
	}
}
...
static struct at24_platform_data at24c512 = {
	...
	/* Most of this EEPROM is unused, but U-Boot uses some data:
 	 ...
 	 */
	.setup		= eeprom_setup_get_mac_addr,
	.context	= (void *)0x10,   ///0x7f00,
};

static struct i2c_board_info __initdata eeprom_i2c_devices[] = {
	{
		I2C_BOARD_INFO("24c512", 0xA0>>1),		// E0=0, E1=0, E2=0
...
static struct disp_syncgen_par mipi_syncgen = {
	...
	.d_vsync_fram		= 0,
	.d_de_cp2			= 7,
	.vs_start_offset	= (CFG_DISP_PRI_HSYNC_FRONT_PORCH + CFG_DISP_PRI_HSYNC_SYNC_WIDTH +
						   CFG_DISP_PRI_HSYNC_BACK_PORCH  + CFG_DISP_PRI_RESOL_WIDTH - 1),
	.ev_start_offset	= (CFG_DISP_PRI_HSYNC_FRONT_PORCH + CFG_DISP_PRI_HSYNC_SYNC_WIDTH +
						   CFG_DISP_PRI_HSYNC_BACK_PORCH  + CFG_DISP_PRI_RESOL_WIDTH - 1),
	.vs_end_offset		= 0,
	.ev_end_offset		= 0,
};
...
void __init nxp_board_devices_register(void)
{
...
#if defined (CONFIG_NXP_DISPLAY_MIPI)
	printk("plat: add mipi display\n");
	nxp_platform_disp_device_data(DISP_DEVICE_MIPI, &mipi_vsync, (void*)&mipi_param, &mipi_syncgen);
#endif
...
#if defined(CONFIG_EEPROM_AT24)
	printk("plat: add device eeprom\n");
	i2c_register_board_info(EEPROM_I2C_BUS, eeprom_i2c_devices, ARRAY_SIZE(eeprom_i2c_devices));
#endif

#if defined(CONFIG_SND_CODEC_WM8976) || defined(CONFIG_SND_CODEC_WM8976_MODULE)
	printk("plat: add device asoc-wm8976\n");
	i2c_register_board_info(WM8976_I2C_BUS, wm8976_i2c_bdi, ARRAY_SIZE(wm8976_i2c_bdi));
	...
	i2c_register_board_info(ALC5623_I2C_BUS, alc5623_i2c_bdi, ARRAY_SIZE(alc5623_i2c_bdi));
	...
	i2c_register_board_info(GSLX680_I2C_BUS, gslX680_i2c_bdi, ARRAY_SIZE(gslX680_i2c_bdi));
#endif

#if defined(CONFIG_TOUCHSCREEN_FT5X0X)
	printk("plat: add touch(ftx0x) device\n");
	i2c_register_board_info(FT5X0X_I2C_BUS, ft5x0x_i2c_bdi, ARRAY_SIZE(ft5x0x_i2c_bdi));
#endif

#if defined(CONFIG_TOUCHSCREEN_FTS)
	printk("plat: add touch(ft54x6) device\n");
	i2c_register_board_info(FT5446_I2C_BUS, ft5446_i2c_bdi, ARRAY_SIZE(ft5446_i2c_bdi));
#endif
#if defined(CONFIG_SENSORS_MMA865X) || defined(CONFIG_SENSORS_MMA865X_MODULE)
	printk("plat: add g-sensor mma865x\n");
	i2c_register_board_info(2, mma865x_i2c_bdi, ARRAY_SIZE(mma865x_i2c_bdi));
#elif defined(CONFIG_SENSORS_MMA7660) || defined(CONFIG_SENSORS_MMA7660_MODULE)
	printk("plat: add g-sensor mma7660\n");
	i2c_register_board_info(MMA7660_I2C_BUS, mma7660_i2c_bdi, ARRAY_SIZE(mma7660_i2c_bdi));
#endif
...
#if defined(CONFIG_INPUT_GPIO_ROTARY_ENCODER)
    platform_device_register(&ak_gpio_encoder_device);  /* [downer] A170921 for GPIO encoder */
#endif
