...
void nxp_cpu_shutdown(void)
{
	...
	//__raw_writel((-1UL), SCR_RESET_SIG_RESET); //yys-reboot
	//__raw_writel(0x0, SCR_RESET_SIG_SET);
...
void nxp_cpu_reset(char str, const char *cmd)
{
  ...
  else if (cmd && !strcmp(cmd, "update")) {
		__raw_writel(UPDATE_SIGNATURE, SCR_RESET_SIG_SET);
		__raw_readl (SCR_RESET_SIG_READ);       /* verify */
	}
	else if (cmd && !strcmp(cmd, "usbboot")) {
		__raw_writel(USBBOOT_SIGNATURE, SCR_RESET_SIG_SET);
		__raw_readl (SCR_RESET_SIG_READ);	/* verify */
	}
	else if (cmd && !strcmp(cmd, "battery_init")) {
		__raw_writel(REBOOT_BATTERY_INIT, SCR_RESET_SIG_SET);
		__raw_readl (SCR_RESET_SIG_READ);	/* verify */
	}
	else{
		__raw_writel(REBOOT_SIGNATURE, SCR_RESET_SIG_SET); //yys-reboot
		__raw_readl (SCR_RESET_SIG_READ);	/* verify */
	}
