...
static bool i2s_ext_is_en(void) {
	printk("Using i2s external clock\n");
	
	return true;
}
	
unsigned long i2s_ext_clk_value;
static unsigned long i2s_set_ext_mclk(unsigned long clk, int ch)
{
	unsigned long ret_clk = 0;
	extern void akaudio_set_mclk(int);
    ...
    if (clk > 1) {
		...
		switch (ch) {
			case 0:
				gpio_request(PAD_GPIO_D + 13, "GPIO_I2S0_MCLK");
...
#if defined(CFG_AUDIO_I2S0_IS_MASTER) && defined(CFG_AUDIO_I2S0_EXTERNAL_MASTER)
	.ext_is_en			= i2s_ext_is_en,
	.set_ext_mclk		= i2s_set_ext_mclk,
#endif
